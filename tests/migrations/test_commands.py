...
from djmodels.db.backends.base.schema import BaseDatabaseSchemaEditor
from djmodels.db.backends.utils import truncate_name
from djmodels.db.migrations.exceptions import InconsistentMigrationHistory
...
class MigrateTests(MigrationTestBase):
    ...
    def test_migrate_fake_initial(self):
        ...
        out = io.StringIO()
        with mock.patch('djmodels.core.management.color.supports_color', lambda *args: False):
    ...
    def test_showmigrations_list(self):
        ...
        """
        out = io.StringIO()
        with mock.patch('djmodels.core.management.color.supports_color', lambda *args: True):
...
class MakeMigrationsTests(MigrationTestBase):
    ...
    def test_makemigrations_empty_connections(self):
        ...
        with mock.patch('djmodels.core.management.commands.makemigrations.connections', new=empty_connections):
            # with no apps
            out = io.StringIO()
    ...
    @mock.patch('djmodels.db.migrations.utils.datetime')
    ...
    @mock.patch('djmodels.db.migrations.questioner.sys.stdin', mock.MagicMock(encoding=sys.getdefaultencoding()))
    def test_makemigrations_auto_now_add_interactive(self, *args):
        ...
        # Monkeypatch interactive questioner to auto accept
        with mock.patch('djmodels.db.migrations.questioner.sys.stdout', new_callable=io.StringIO) as prompt_stdout:
            out = io.StringIO()
...
class AppLabelErrorTests(TestCase):
    ...
    app. 'djmodels.contrib.auth' must be in INSTALLED_APPS for some of these
    tests.
    """
    nonexistent_app_error = "No installed app with label 'nonexistent_app'."
    did_you_mean_auth_error = (
        "No installed app with label 'djmodels.contrib.auth'. Did you mean "
        "'auth'?"
    )
    ...
    def test_makemigrations_app_name_specified_as_label(self):
        err = io.StringIO()
        with self.assertRaises(SystemExit):
            call_command('makemigrations', 'djmodels.contrib.auth', stderr=err)
    ...
    def test_migrate_app_name_specified_as_label(self):
        with self.assertRaisesMessage(CommandError, self.did_you_mean_auth_error):
            call_command('migrate', 'djmodels.contrib.auth')
    ...
    def test_sqlmigrate_app_name_specified_as_label(self):
        with self.assertRaisesMessage(CommandError, self.did_you_mean_auth_error):
            call_command('sqlmigrate', 'djmodels.contrib.auth', '0002')
    ...
    def test_squashmigrations_app_name_specified_as_label(self):
        with self.assertRaisesMessage(CommandError, self.did_you_mean_auth_error):
            call_command('squashmigrations', 'djmodels.contrib.auth', '0002')
